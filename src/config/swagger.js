...
const swaggerSpec = {
  ...
  tags: [
    ...
    { name: 'Beneficiarios Retornos', description: 'Montos de retorno por beneficiario' },
    { name: 'Pagos Beneficiarios', description: 'Pagos desde cuenta empresa Austral' },
    { name: 'Solicitud Retorno', description: 'Devolución de la solicitud' },
    { name: 'Pagos Teniente', description: 'Entrega física de efectivo por teniente' },
    { name: 'Pagos Teniente Historial', description: 'Historial de validaciones y entregas' },
  ],
  components: {
    securitySchemes: {
      bearerAuth: {
    ...
    schemas: {
        ...
        allOf: [
          { $ref: '#/components/schemas/Solicitud' },
          {
            ...
            properties: {
              ...
              pagos_teniente: {
                type: 'array',
                items: { $ref: '#/components/schemas/PagoTeniente' },
              },
            },
          },
        ],
      ...
      PagoTeniente: {
        ...
        properties: {
          id: { type: 'string', format: 'uuid' },
          pago_beneficiario_id: { type: 'string', format: 'uuid' },
          beneficiario_id: { type: 'string', format: 'uuid' },
          solicitud_id: { type: 'string', format: 'uuid' },
          intentos_fallidos: { type: 'integer', example: 0 },
          max_intentos: { type: 'integer', example: 3 },
          bloqueado_hasta: { type: 'string', format: 'date-time', nullable: true },
          entregado_por: { type: 'string', format: 'uuid' },
          receptor_tipo: { type: 'string', enum: ['beneficiario', 'tercero'], example: 'beneficiario' },
          receptor_nombre: { type: 'string', nullable: true },
          identificacion_receptor: { type: 'string', nullable: true },
          receptor_firma_url: { type: 'string', nullable: true },
          foto_comprobante_url: { type: 'string', nullable: true },
          latitud: { type: 'number', nullable: true, example: 19.432608 },
          longitud: { type: 'number', nullable: true, example: -99.133209 },
          estado_id: { type: 'integer', example: 1 },
          observaciones: { type: 'string', nullable: true },
          monto_pagado: { type: 'string', nullable: true },
        },
      },
      CreatePagoTenienteRequest: {
        type: 'object',
        required: ['pago_beneficiario_id', 'beneficiario_id', 'entregado_por'],
        properties: {
          pago_beneficiario_id: { type: 'string', format: 'uuid' },
          beneficiario_id: { type: 'string', format: 'uuid' },
          entregado_por: { type: 'string', format: 'uuid', description: 'user_id del teniente' },
          receptor_tipo: { type: 'string', enum: ['beneficiario', 'tercero'] },
          max_intentos: { type: 'integer', minimum: 1, maximum: 10, example: 3 },
          estado_id: { type: 'integer', example: 1 },
          observaciones: { type: 'string', nullable: true },
        },
      },
      UpdatePagoTenienteRequest: {
        type: 'object',
        properties: {
          entregado_por: { type: 'string', format: 'uuid' },
          receptor_tipo: { type: 'string', enum: ['beneficiario', 'tercero'] },
          ...
          estado_id: { type: 'integer' },
          max_intentos: { type: 'integer' },
          observaciones: { type: 'string', nullable: true },
        },
      },
      ValidarCodigoPagoTenienteRequest: {
        type: 'object',
        required: ['codigo'],
        properties: {
      ...
      EntregarPagoTenienteRequest: {
        type: 'object',
        required: ['latitud', 'longitud'],
        properties: {
          codigo: { type: 'string', pattern: '^\\d{6}$', description: 'Requerido si no hay validación reciente' },
          receptor_tipo: { type: 'string', enum: ['beneficiario', 'tercero'] },
          receptor_nombre: { type: 'string', nullable: true },
          identificacion_receptor: { type: 'string', nullable: true },
          receptor_firma_url: { type: 'string', nullable: true },
          foto_comprobante_url: { type: 'string', nullable: true },
          latitud: { type: 'number', example: 19.432608 },
          longitud: { type: 'number', example: -99.133209 },
          observaciones: { type: 'string', nullable: true },
        },
      },
      PagoTenienteHistorial: {
        type: 'object',
        properties: {
          id: { type: 'string', format: 'uuid' },
          pago_teniente_id: { type: 'string', format: 'uuid' },
          estado_anterior_id: { type: 'integer', nullable: true },
          estado_nuevo_id: { type: 'integer' },
          evento: {
            type: 'string',
            enum: ['CREACION', 'VALIDACION_CODIGO', 'ENTREGA', 'ACTUALIZACION', 'DESACTIVACION', 'CAMBIO_ESTADO'],
          },
          codigo_proporcionado: { type: 'string', nullable: true },
          es_correcto: { type: 'boolean', nullable: true },
          cambiado_por: { type: 'string', format: 'uuid' },
          cambiado_en: { type: 'string', format: 'date-time' },
          detalles: { type: 'object', nullable: true },
        },
      },
      CreatePagoTenienteHistorialRequest: {
        type: 'object',
        required: ['pago_teniente_id', 'estado_nuevo_id'],
        properties: {
          pago_teniente_id: { type: 'string', format: 'uuid' },
          estado_anterior_id: { type: 'integer', nullable: true },
          estado_nuevo_id: { type: 'integer' },
  ...
  paths: {
    ...
    '/pagos-teniente': {
      get: {
        tags: ['Pagos Teniente'],
        summary: 'Listar pagos teniente',
        description: 'TENIENTE solo ve sus propios pagos (entregado_por = su user_id).',
        security: [{ bearerAuth: [] }],
        parameters: [
          { name: 'solicitud_id', in: 'query', schema: { type: 'string', format: 'uuid' } },
          { name: 'beneficiario_id', in: 'query', schema: { type: 'string', format: 'uuid' } },
          { name: 'estado_id', in: 'query', schema: { type: 'integer' } },
          { name: 'entregado_por', in: 'query', schema: { type: 'string', format: 'uuid' } },
      ...
      post: {
        tags: ['Pagos Teniente'],
        summary: 'Crear pago teniente (ADMIN)',
        description: 'Genera código de verificación de 6 dígitos. Un registro por pago_beneficiario (unique).',
        security: [{ bearerAuth: [] }],
        requestBody: {
          required: true,
          content: { 'application/json': { schema: { $ref: '#/components/schemas/CreatePagoTenienteRequest' } } },
        },
        responses: {
          201: { description: 'Creado (incluye codigo_verificacion en la respuesta)' },
          409: { description: 'Ya existe pago teniente para ese pago a beneficiario' },
        },
      },
    },
    '/pagos-teniente/{id}': {
      ...
      put: {
        tags: ['Pagos Teniente'],
        summary: 'Actualizar pago teniente',
        security: [{ bearerAuth: [] }],
        parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'string', format: 'uuid' } }],
        requestBody: {
          required: true,
          content: { 'application/json': { schema: { $ref: '#/components/schemas/UpdatePagoTenienteRequest' } } },
        },
        responses: { 200: { description: 'Actualizado' } },
      },
      delete: {
        tags: ['Pagos Teniente'],
        summary: 'Desactivar pago teniente (ADMIN)',
        security: [{ bearerAuth: [] }],
        parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'string', format: 'uuid' } }],
        responses: { 200: { description: 'Desactivado' } },
      },
    },
    '/pagos-teniente/{id}/validar-codigo': {
      post: {
        tags: ['Pagos Teniente'],
        summary: 'Validar código de verificación',
        description: 'Máx. intentos configurables (default 3). Bloqueo 30 min al exceder.',
        security: [{ bearerAuth: [] }],
        parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'string', format: 'uuid' } }],
        requestBody: {
          required: true,
          content: { 'application/json': { schema: { $ref: '#/components/schemas/ValidarCodigoPagoTenienteRequest' } } },
        },
        responses: {
          200: { description: 'Código correcto' },
          400: { description: 'Código incorrecto' },
          429: { description: 'Bloqueado por intentos fallidos' },
        },
      },
    },
    '/pagos-teniente/{id}/entregar': {
      post: {
        tags: ['Pagos Teniente'],
        summary: 'Registrar entrega con geolocalización',
        description: 'Requiere latitud/longitud. Valida código si no hubo validación exitosa en los últimos 30 min.',
        security: [{ bearerAuth: [] }],
        parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'string', format: 'uuid' } }],
        requestBody: {
          required: true,
          content: { 'application/json': { schema: { $ref: '#/components/schemas/EntregarPagoTenienteRequest' } } },
        },
        responses: { 200: { description: 'Entrega registrada (estado PAGADO)' } },
      },
    },
    '/pagos-teniente-historial': {
      get: {
        tags: ['Pagos Teniente Historial'],
        summary: 'Listar historial de pagos teniente',
        security: [{ bearerAuth: [] }],
        parameters: [
          { name: 'pago_teniente_id', in: 'query', schema: { type: 'string', format: 'uuid' } },
          {
            name: 'evento',
            in: 'query',
            schema: {
      ...
      post: {
        tags: ['Pagos Teniente Historial'],
        summary: 'Crear registro de historial (ADMIN)',
        security: [{ bearerAuth: [] }],
        requestBody: {
          required: true,
          content: { 'application/json': { schema: { $ref: '#/components/schemas/CreatePagoTenienteHistorialRequest' } } },
        },
        responses: { 201: { description: 'Creado' } },
      },
    },
    '/pagos-teniente-historial/{id}': {
      get: {
        tags: ['Pagos Teniente Historial'],
        summary: 'Obtener registro por ID',
        security: [{ bearerAuth: [] }],
